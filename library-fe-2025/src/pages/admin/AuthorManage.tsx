...
export default function AuthorManage() {
    ...
    const handleSubmit = async (name: string) => {
        if (editingItem) await AuthorService.update(editingItem.id, name);
        else await AuthorService.create(name);
        fetch();
    };
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-purple-50 p-6">
                <div className="max-w-7xl mx-auto">
                    <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                        <div>
                            <h2 className="text-2xl font-bold text-purple-800">Authors</h2>
                            <p className="text-gray-600 text-sm mt-1">Total: {items.length} authors</p>
                        </div>
                        <div className="flex gap-3 w-full md:w-auto">
                            <input type="text" placeholder="Search authors..." className="px-4 py-2 border border-purple-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-400 w-full md:w-64 shadow-sm"
                    ...
                    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
                        <table className="w-full text-left">
                            ...
                            {loading ? <tr><td colSpan={3} className="text-center py-8 text-gray-500">Loading...</td></tr> :
                                filteredItems.length === 0 ? <tr><td colSpan={3} className="text-center py-8 text-gray-500">No authors found.</td></tr> :
            ...
            <AuthorFormModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={handleSubmit} initialData={editingItem} />
        </>
    );
}